    ...
    <div class="wrapper clearfix">
        ...
        <div class="content">
                ...
                <table class="table table-bordered mt-4">
                    ...
                    <tbody>
                        <c:forEach var="cmt" items="${comments}">
                            <tr>
                                <td>${cmt.feedbackID}</td>
                                <td>${cmt.customerName}</td>
                                <td>${cmt.comment}</td>
                                <td><fmt:formatDate value="${cmt.createAt}" pattern="dd-MM-yyyy"/></td>
                                <td>
                                    <a href="FeedbackController?action=admin-delete-feedback&fid=${cmt.feedbackID}&productID=${productID}" 
                                       class="btn btn-danger btn-sm" 
                                       onclick="return confirm('Are you sure you want to delete this comment?');">Delete</a>
                                    <a href="FeedbackController?action=reply&fid=${cmt.feedbackID}&productID=${productID}" 
                                       class="btn btn-primary btn-sm ms-2">Reply</a>
                                </td>
                            </tr>
                        </c:forEach>
                    </tbody>
            ...
            <c:if test="${empty comments}">
                <div class="alert alert-warning mt-4">No feedback available for this product.</div>
            </c:if>

            <a href="ProductController" class="btn btn-secondary mt-3">Back to Product List</a>
        </div>
    </div>
