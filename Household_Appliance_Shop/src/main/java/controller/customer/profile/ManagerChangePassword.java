...
HttpSession session = request.getSession();
        ...
        if (!newPassword.matches(passwordPattern)) {
            request.setAttribute("error", "The password must contain at least one letter, one number, one special character, and be at least 8 characters long!");
            request.getRequestDispatcher("managerchangepassword.jsp").forward(request, response);
            return;
        }
        if (!newPassword.equals(confirmPassword)) {
            request.setAttribute("error", "The new password and confirmation do not match!");
            request.getRequestDispatcher("managerchangepassword.jsp").forward(request, response);
            return;
        }
        ...
        try {
            ...
            if (isChanged) {
                ...
                request.setAttribute("error", "Incorrect old password or an error occurred!");
            }
